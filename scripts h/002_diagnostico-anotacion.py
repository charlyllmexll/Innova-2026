...
IMAGES_DIR     = r"C:\Users\charl\Downloads\Fotos Drone 50mts Abr 15\Muestra"
...
def process_image(image_path, mask_generator, split="train"):
    ...
    all_dets     = []
    total_raw    = 0  # total de máscaras que genera SAM
    total_valid  = 0  # cuántas pasan todos los filtros
    ...
    print(f"  [{os.path.basename(image_path)}] SAM generó {total_raw} máscaras → {total_valid} pasaron filtros")

    # NMS sobre bounding boxes (ligero, sin máscaras completas)
    print(f"  [{os.path.basename(image_path)}] Detecciones antes de NMS: {len(all_dets)}")
    all_dets = apply_nms(all_dets)
    print(f"  [{os.path.basename(image_path)}] Detecciones después de NMS: {len(all_dets)}")
